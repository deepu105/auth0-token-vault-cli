  ...
  draft
    ...
    .action(async (opts, cmd: Command) => {
      try {
        ...
        if (!body) {
          outputError(
            { code: 'missing_body', message: 'Draft body required. Use --body, --body-file, or pipe via stdin.' },
            cmd
          );
        ...
        output(
          { data: result },
          chalk.green(`Draft created (id: ${result.id})`),
          cmd
        );
      } catch (err) {
  ...
  draft
    ...
    .action(async (draftId: string, _opts, cmd: Command) => {
      try {
        ...
        output(
          { data: result },
          chalk.green(`Draft sent (id: ${result.id})`),
          cmd
        );
      } catch (err) {
        handleGmailError(err, cmd);
  ...
  draft
    ...
    .action(async (draftId: string, _opts, cmd: Command) => {
      try {
        ...
        output(
          { status: 'deleted', draftId },
          chalk.green(`Draft ${draftId} deleted.`),
          cmd
        );
      } catch (err) {
        handleGmailError(err, cmd);
